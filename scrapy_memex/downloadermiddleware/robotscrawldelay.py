...
class RobotsCrawlDelayMiddleware(object):
    ...
    Settings to enable this middleware::
        ...
        # currently autothrottle doesn't play well with
    ...
    def __init__(self, crawler):
        if not crawler.settings.getbool('ROBOTS_CRAWLDELAY_ENABLED'):
            raise NotConfigured
        self.verbose = crawler.settings.getbool('ROBOTS_CRAWLDELAY_VERBOSE',
                                                False)
    ...
    def _adjust_delay(self, rules, request_or_response, spider):
        ...
        if delay != slot.delay:
            log.msg("Adjusting delay for %s: %0.2f -> %0.2f" %
                    (key, slot.delay, delay), logging.DEBUG)
            slot.delay = delay

        if self.verbose:
            self.crawler.stats.set_value('robots.txt/crawl-delay/%s' %
                                         key, delay)
    ...
    def _get_slot(self, request_or_response, spider):
        ...
        return self.crawler.engine.downloader._get_slot(request_or_response,
                                                        spider)
